...
features = no_zeros.loc[:, features_list]
target = no_zeros.loc[:, 'vb_slice']
...
history = model.fit(X_train_scaled, y_train, validation_data = (X_val_scaled, y_val), verbose = True)#, callbacks = [reduce_lr])
cross_val = cross_val_score(model, X_train_scaled, y_train, cv=5, verbose=10, scoring=make_scorer(r2_score))

# # Save training results
with open('results/naive_dropout_history.pkl' , 'wb') as handle:
    pk.dump(history.history, handle, protocol=pk.HIGHEST_PROTOCOL)
with open('results/naive_dropout_crossval.pkl', 'wb') as handle:
    pk.dump(cross_val, handle, protocol=pk.HIGHEST_PROTOCOL)
model.model.save(r'results/naive_dropout_model.h5')

y_pred_val = model.predict(X_val_scaled)
r2_score_val = r2_score(y_val, y_pred_val)
print("R2 validação: ", r2_score_val) 
##############################################################################
# END MODEL
###############################################################################
...
fig.savefig('plots/history_evolution_naive_dropout.pdf', format='pdf')
plt.show()
