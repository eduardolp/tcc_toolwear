...
bins=[0,0.05,0.1,0.15,0.2,max(df.vb_slice)]
...
# with open('results/rus_notmin_history.pkl' , 'wb') as handle:
#     pk.dump(history.history, handle, protocol=pk.HIGHEST_PROTOCOL)
# with open('results/rus_notmin_crossval.pkl', 'wb') as handle:
#     pk.dump(cross_val, handle, protocol=pk.HIGHEST_PROTOCOL)
# model.model.save(r'results/rus_notmin_model.h5')
...
# fig.savefig('plots/history_evolution_rus_notmin.pdf', format='pdf')
plt.show()