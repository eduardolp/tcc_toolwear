...
target_reg = no_zeros.loc[:, 'vb_slice']
...
X_train_reg, X_partial_reg, y_train_reg, y_partial_reg = train_test_split(features, target_reg, 
                                                    test_size=0.3, random_state=42)
X_val_reg, X_test_reg, y_val_reg, y_test_reg = train_test_split(X_partial_reg, y_partial_reg, 
...
X_train_reduced_reg = X_train_reg.loc[:, features_list]
y_train_reduced_reg = y_train_reg 
...
X_val_reduced_reg = X_val_reg.loc[:, features_list]
y_val_reduced_reg = y_val_reg
X_val_reduced3 = X_val3.loc[:, features_list]
y_val_reduced3 = y_val3
X_val_reduced5 = X_val5.loc[:, features_list]
y_val_reduced5 = y_val5

X_test_reduced_reg = X_test_reg.loc[:, features_list]
y_test_reduced_reg = y_test_reg
...
X_scaler_reg = preprocessing.StandardScaler().fit(X_train_reduced_reg)
X_train_scaled_reg = X_scaler_reg.transform(X_train_reduced_reg)
X_val_scaled_reg = X_scaler_reg.transform(X_val_reduced_reg)
X_test_scaled_reg = X_scaler_reg.transform(X_test_reduced_reg)
...
model_reg = load_model(r"results\naive_model.h5", custom_objects={'r2_keras':r2_keras})
model3 = load_model(r"results\naive_class3_model.h5")
model5 = load_model(r"results\naive_class5_model_2tentativa.h5")

# reads the training history. Useful for plotting the loss evolution during training      
with open(r"results\naive_history.pkl", 'rb') as handle:
    history_reg = pk.load(handle)
...
with open(r"results\naive_crossval.pkl", 'rb') as handle:
    crossval_reg = pk.load(handle)
...
list_models_reg = [model_reg]
list_history_reg = [history_reg]
list_crossval_reg = [crossval_reg]
...
# fig.savefig('plots/history_evolution_35classes_naive_correcao.pdf', format='pdf')

###################################################################################
# Test data evaluation
y_pred_reg = pd.DataFrame(model_reg.predict(X_test_scaled_reg), columns=['vb_slice'])
y_pred_reg['vb_sliceRANGE3'] = pd.cut(y_pred_reg.vb_slice, bins=bins3, labels=names3, include_lowest=True)
y_pred_reg['vb_sliceRANGE5'] = pd.cut(y_pred_reg.vb_slice, bins=bins5, labels=names5, include_lowest=True)

acc3_reg = accuracy_score(encoder3.inverse_transform(y_test_reduced3.argmax(1)),y_pred_reg.vb_sliceRANGE3)
print("Acurácia regressão 3 classes: ", acc3_reg) 

acc5_reg = accuracy_score(encoder5.inverse_transform(y_test_reduced5.argmax(1)), y_pred_reg.vb_sliceRANGE5)
print("Acurácia regressão 5 classes: ", acc5_reg)
...
report3_reg = classification_report(encoder3.inverse_transform(y_test_reduced3.argmax(1)), y_pred_reg.vb_sliceRANGE3, labels=names3)
confusion3_reg = confusion_matrix(encoder3.inverse_transform(y_test_reduced3.argmax(1)), y_pred_reg.vb_sliceRANGE3, labels=names3, normalize='true')

report5_reg = classification_report(encoder5.inverse_transform(y_test_reduced5.argmax(1)), y_pred_reg.vb_sliceRANGE5, labels=names5)
confusion5_reg = confusion_matrix(encoder5.inverse_transform(y_test_reduced5.argmax(1)), y_pred_reg.vb_sliceRANGE5, labels=names5, normalize='true')
