...
# borders = [pd.Interval(left=0, right=0.05),pd.Interval(left=0.05, right=0.2),pd.Interval(left=0.2, right=1) ]
# name = ('low', 'medium', 'high')
# intervals = pd.IntervalIndex(borders, name=name)
# Creates categories based on vb_slice 
def categorizer(dataframe, interval):
    ...
    text = 'ABCDEFGHIJ'
    labels = list(text)
    dataframe['vb_sliceRANGE'], intervals = pd.cut(dataframe.vb_slice, 10, retbins=True)
    # counter
    ...
    return  dataframe, group_size, intervals
...
no_zeros_cat['vb_sliceRANGE'] , interval= pd.cut(no_zeros.vb_slice, bins=[0,0.02,0.04,0.06,0.08,0.1,0.12,0.14,0.16,0.18,0.2,1],labels=['a','b','c','d','e','f','g','h','i','j','k'], retbins=True)
...
hidden_layer7 = Dense(10, activation = ac_hid_, 
                      kernel_regularizer = regularizers.l1_l2(0,0))
output_layer = Dense(11, activation = ac_out_, 
                      kernel_regularizer = regularizers.l1_l2(0,0))
...
report = classification_report(encoder.inverse_transform(y_val_reduced.argmax(1)), encoder.inverse_transform(y_pred), labels=('a','b','c','d','e','f','g','h','i','j','k'))
confusion = confusion_matrix(encoder.inverse_transform(y_val_reduced.argmax(1)), encoder.inverse_transform(y_pred), labels=('a','b','c','d','e','f','g','h','i','j','k'), normalize='true')
