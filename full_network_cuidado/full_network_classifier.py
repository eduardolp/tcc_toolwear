...
hidden_layer7 = Dense(10, activation = ac_hid_, 
                      kernel_regularizer = regularizers.l1_l2(0,0))
output_layer = Dense(10, activation = ac_out_, 
                      kernel_regularizer = regularizers.l1_l2(0,0))
...
history = model.fit(X_train_scaled, np.array(y_train_reduced).reshape(1,-1), validation_data = (X_val_scaled, np.array(y_val_reduced).reshape(1,-1)), verbose = True)#, callbacks = [reduce_lr])
