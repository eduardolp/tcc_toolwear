...
with open('results/naive_history.pkl' , 'wb') as handle:
    pk.dump(history.history, handle, protocol=pk.HIGHEST_PROTOCOL)
with open('results/naive_crossval.pkl', 'wb') as handle:
    pk.dump(cross_val, handle, protocol=pk.HIGHEST_PROTOCOL)
model.model.save(r'results/naive_model.h5')
...
# fig.savefig('plots/history_evolution_naive.pdf', format='pdf')
plt.show()