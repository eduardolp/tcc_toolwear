...
from sklearn.metrics import make_scorer, r2_score, classification_report, confusion_matrix, roc_auc_score, accuracy_score
...
with open('results/naive_class5_history20200513.pkl' , 'wb') as handle:
    pk.dump(history.history, handle, protocol=pk.HIGHEST_PROTOCOL)
with open('results/naive_class5_crossval20200513.pkl', 'wb') as handle:
    pk.dump(cross_val, handle, protocol=pk.HIGHEST_PROTOCOL)
model.model.save(r'results/naive_class5_model20200513.h5')
...
y_pred = pd.DataFrame(model.predict(X_test_scaled), columns=['vb_slice'])
...
y_pred5 = model.predict(X_test_scaled)
acc5 = accuracy_score(encoder.inverse_transform(y_test_reduced.argmax(1)), encoder.inverse_transform(y_pred5))
