...
def prepare_data(dataframe, plot=False):
    ...
    Work in progress: make so that each vb_slice range has the same or similar
    amount of data
    '''
    df = dataframe
    ...
    text = 'ABCDEFGHIJ'
    labels = list(text)
    no_zeros['vb_sliceRANGE'] = pd.cut(no_zeros.vb_slice, 10, labels=labels)
    
    cat_group = no_zeros.groupby('vb_sliceRANGE') #the name of each group is given by the category label
    ...
    leveled_df = no_zeros.sample(frac=1).groupby('vb_sliceRANGE').head(2*min(group_size))
    ...
    # dataframes. 
    reduced_df = leveled_df.loc[:,['engagements','hardness','bsp','lsp','vc','vb_slice']]
...
# no_zeros = prepare_data(grouped.get_group('04'))
...
def train_model(dataframe):
    ...
    with open('article_networks_leveled/poly_reg_2x/article%s_history.pkl' %dataframe_number, 'wb') as handle:
        pk.dump(history.history, handle, protocol=pk.HIGHEST_PROTOCOL)
    with open('article_networks_leveled/poly_reg_2x/article%s_crossval.pkl' %dataframe_number, 'wb') as handle:
        pk.dump(cross_val, handle, protocol=pk.HIGHEST_PROTOCOL)
    model.model.save(r'article_networks_leveled/poly_reg_2x/article%s_model.h5' %dataframe_number)
